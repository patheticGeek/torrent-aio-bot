...
const { uploadFolder } = require("../utils/gdrive");
...
class Torrent {
  constructor() {
    ...
    setInterval(() => {
      this.downloads = this.client.torrents.map(torrent =>
        this.get(torrent.magnetURI)
      );
    }, 3000);
  }
  ...
  saveFiles = async torrent => {
    ...
    // await ziper(`./downloads/${torrent.infoHash}/${torrent.name}`);
    uploadFolder(`./downloads/${torrent.infoHash}/${torrent.name}`);
  };
}
