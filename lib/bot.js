...
function bot(torrent, bot) {
        ...
        [
          {
            ...
            url: 'https://t.me/dx_botz'
          }
        ],
        [
          {
            text: 'support Group ✅',
            url: 'https://t.me/dx_support'
          }
        ]
      ]
    },
  ...
  bot.sendMessage(msg.from.id, "*Hai !!.*\n\n_Welcome to TorrentDx Bot..Am a Assistant bot of_ [👉 this web 🔥](https://torrentdxbot.herokuapp.com/). _Magnet links convert to direct link & Upload our team drive 😎_\n\n*☛  Do one By One . Otherwise you will get Permenent Ban*\n\n*☛ porn or 18+ series strictly prohobited*\n\n*☛ /help for more details..*", opts);
  });
  ...
  bot.on('callback_query', async callbackQuery => {
    ...
    if (action === 'drive') {
      text = 'https://telegra.ph/How-to-join-torrent-dx-team-drive-07-27';
    }
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const reply = async torr => {
      ...
      if (!torr.done) {
        mess1 += `Done ✅ : ${torr.downloaded}\n`;
        mess1 += `Speed 🚀: ${torr.speed}\n\n`;
       
        mess1 += `Left  🕔 : ${torr.redableTimeRemaining}\n\n`;
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(from, "Removed successfully");
    } catch (e) {
