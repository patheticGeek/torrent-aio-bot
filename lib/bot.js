...
/status {infoHash} - To check status of a downloading torrent
info hash is provided when torent download starts
eg.
    /status 08ada5a7a6183aae1e09d831df6748d566095a10

/remove {infoHash} - To remove an already added torrent
eg.
    /remove 08ada5a7a6183aae1e09d831df6748d566095a10
`;
      ...
      try {
        torrent.download(
          ...
          torr =>
            bot.sendMessage(
              from,
              `Name: ${torr.name}\n\nStatus: ${torr.status}\n\nSize: ${torr.total}\n\nInfo Hash: ${torr.infoHash}`
            ),
          torr =>
            bot.sendMessage(
              from,
              `Name: ${torr.name}\n\nStatus: ${torr.status}\n\nSize: ${torr.total}\n\nLink: ${torr.downloadLink}\n\nInfo Hash: ${torr.infoHash}`
            )
        );
      } catch (e) {
        bot.sendMessage(from, "An error occured\n" + e.message);
