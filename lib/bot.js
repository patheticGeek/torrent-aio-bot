...
function bot(torrent, bot) {
  ...
  bot.on("message", async msg => {
    ...
    try {
      ...
      bot.sendMessage(chatId, `${fileName} upload successfull\nDrive link: ${driveLink}\nPublic link: ${publicLink}`);
    } catch (e) {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      try {
        ...
        torrInterv = setInterval(async () => {
          ...
          if (messageObj) bot.editMessageText(mess1, { chat_id: messageObj.chat.id, message_id: messageObj.message_id });
